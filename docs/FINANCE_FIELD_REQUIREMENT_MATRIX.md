...
| Money Transfer | Transfer type; date; amount sent; sending currency; source account; destination account; receiving currency | Internal: accounts must differ and stay in the selected country. Cross-country: both-country access, direction-matched accounts, transfer service, and actual amount received are required. Actual amounts create immutable effective-rate snapshots. A fee, when entered, must use the source account currency | Provider quoted rate; transfer fee; Other service name unless Other is selected; reference; purpose; notes; receipt |
