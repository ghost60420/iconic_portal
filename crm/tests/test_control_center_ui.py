...
class ControlCenterUITests(TestCase):
    ...
    def test_ceo_dashboard_uses_control_center_shell_and_preserves_filters(self):
        ...
        self.assertContains(response, 'href="#control-overview"')
        self.assertContains(response, 'href="#control-operations"')
        self.assertContains(response, 'href="#control-integrations"')
        self.assertContains(response, 'href="#control-automations"')
        self.assertContains(response, 'href="#control-logs"')
        self.assertContains(response, 'href="#control-history"')
        self.assertContains(response, 'id="days"')
        self.assertContains(response, 'name="days"')
        self.assertContains(response, 'id="side"')
        self.assertContains(response, 'name="side"')
        self.assertContains(response, "ceo-dashboard-data")
        self.assertContains(response, 'data-ceo-chart="production-status"')
