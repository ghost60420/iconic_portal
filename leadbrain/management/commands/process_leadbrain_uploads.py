...
def _truncate_url(value: str) -> str:
    return (value or "")[:200]


def _update_upload_note(upload: LeadBrainUpload) -> None:
...
def _select_batch(upload: LeadBrainUpload, batch_size: int) -> list[int]:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        uploads = LeadBrainUpload.objects.order_by("uploaded_at", "id")
        if upload_id:
        ...
        for upload in uploads[:limit]:
            ...
            _update_upload_note(upload)

            while True:
                batch_ids = _select_batch(upload, batch_size)
                if not batch_ids:
                    upload.refresh_progress()
                    _update_upload_note(upload)
                    break
                ...
                for company in LeadBrainCompany.objects.filter(id__in=batch_ids).order_by("row_number", "id"):
                    try:
                        ...
                        company.website = _truncate_url(
                            company.website or research_data.get("official_website_found", "")
                        )
                        ...
                        company.linkedin_url = _truncate_url(research_data.get("linkedin_url_found", ""))
